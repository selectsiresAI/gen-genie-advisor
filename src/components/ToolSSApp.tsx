...
function CalcPage({ farm, weights, setWeights, bulls, onBack }: any) {
  const [w, setW] = useState<Weights>(weights);
  useEffect(() => setW(weights), [weights]);

  // Ranking de melhor touro por fêmea (com base no índice custom)
  const stats = useMemo(() => {
    const cols = ["TPI", "NM$", "Milk", "Fat", "Protein", "SCS", "PTAT"] as const;
    const s: any = {};
    cols.forEach((c) => (s[c === "NM$" ? "NM" : c] = zscale(bulls.map((b: any) => Number(b[c])))));
    return { TPI: s.TPI, NM: s.NM, Milk: s.Milk, Fat: s.Fat, Protein: s.Protein, SCS: s.SCS, PTAT: s.PTAT };
  }, [bulls]);

  const ranked = useMemo(() => {
    return bulls
      .map((b: any) => ({ ...b, _score: scoreAnimal(b as any, stats, w) }))
      .sort((a: any, b: any) => b._score - a._score);
  }, [bulls, stats, w]);

  const bestBull = ranked[0];

  const matches = useMemo(() => {
    if (!bestBull) return [];
    return farm.females.map((f: Female) => {
      const d = projectDaughter(f, bestBull);
      return {
        brinco: f.brinco,
        naab_touro: bestBull.naab,
        touro: bestBull.nome,
        ganho_TPI: d.TPI - f.TPI,
        ganho_NM$: d.NM$ - f.NM$,
        ganho_Leite: d.Milk - f.Milk,
        proj_TPI: d.TPI,
        proj_NM$: d.NM$,
        proj_Leite: d.Milk,
      };
    });
  }, [farm.females, bestBull]);

  const WeightInput = ({ k, label }: any) => (
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6">
      ...
      <div className="grid md:grid-cols-3 gap-6">
        <Card>
          <CardHeader>
            <CardTitle>Ajuste de pesos (índice)</CardTitle>
          </CardHeader>
          <CardContent className="space-y-2">
            <WeightInput k="TPI" label="TPI" />
            <WeightInput k="NM$" label="NM$" />
            <WeightInput k="Milk" label="Leite (lbs)" />
            <WeightInput k="Fat" label="Gordura (lbs)" />
            <WeightInput k="Protein" label="Proteína (lbs)" />
            <WeightInput k="PTAT" label="PTAT (Tipo)" />
            <WeightInput k="SCS" label="CCS (penaliza)" />
            <div className="flex gap-2 mt-3">
              <Button onClick={() => setWeights(w)} size="sm">
                Aplicar ao sistema
              </Button>
              <Button onClick={() => toCSV(matches, "predicao_por_femea.csv")} size="sm">
                <Download size={16} className="mr-2" /> Exportar predição
              </Button>
            </div>
            <div className="text-xs text-muted-foreground">
              *Predição simplificada por Parent Average (mãe+touro)/2 apenas para MVP.
            </div>
          </CardContent>
        </Card>
        ...
        <div className="md:col-span-2">
          <Card>
            <CardHeader>
              <CardTitle>Top touros pelo índice</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="overflow-auto rounded-lg border">
                <table className="min-w-[700px] w-full">
                  <thead>
                    <tr>
                      {["NAAB","Nome","TPI","NM$","Milk","Fat","Protein","SCS","PTAT","Score"].map((h) => (
                        <th key={h} className="px-3 py-2 sticky top-0 bg-foreground text-background">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {ranked.slice(0, 8).map((b: any) => (
                      <tr key={b.naab} className="odd:bg-card even:bg-muted/50">
                        <td className="px-3 py-2">{b.naab}</td>
                        ...
                        <td className="px-3 py-2">{b.SCS.toFixed(2)}</td>
                        <td className="px-3 py-2">{b.PTAT.toFixed(2)}</td>
                        <td className="px-3 py-2 font-bold">{b._score.toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              {ranked[0] && (
                <div className="mt-4 text-sm text-muted-foreground">
                  <span className="font-semibold">Melhor por índice (MVP):</span>{" "}
                  {ranked[0].naab} — {ranked[0].nome}. Use a aba <b>Gráficos</b> para ver o impacto médio projetado no rebanho.
                </div>
              )}
            </CardContent>
          </Card>
