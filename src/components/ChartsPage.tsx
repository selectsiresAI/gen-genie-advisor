...
import { ArrowLeft, Settings, Download, RefreshCw, Users, TrendingUp, BarChart3, PieChart as PieChartIcon, Activity, Eye, EyeOff } from "lucide-react";
...
const ChartsPage: React.FC<ChartsPageProps> = ({ farm, onBack, onNavigateToHerd }) => {
  ...
  const calculateStatistics = (_rows: any[]) => {};
  ...
  const loadFemalesData = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from('females_denorm')
        .select('birth_date, tpi, hhp_dollar, nm_dollar, cm_dollar, fm_dollar, gm_dollar, category, parity_order, farm_id')
      ...
      if (process.env.NODE_ENV !== 'production') {
        console.info('[Tendências][QUERY] farmId', farm?.farm_id);
        console.info('[Tendências][QUERY] rows', sorted.length);
        if (sorted[0]) console.info('[Tendências][QUERY] sample keys', Object.keys(sorted[0]).slice(0, 25));
      }
    } catch (err: any) {
  ...
  const validSelectedPTAs = useMemo(() => {
    if (!Array.isArray(females) || females.length === 0) return [];

    const existingKeys = new Set(Object.keys(females[0] || {}));
    const filtered = normalizedPTAs.filter((key) => existingKeys.has(key));

    if (filtered.length === 0 && process.env.NODE_ENV !== 'production') {
      console.info('[Tendências] Nenhuma PTA encontrada nas colunas', {
        selectedPTAs,
        normalizedPTAs,
        sampleKeys: Array.from(existingKeys).slice(0, 25),
      });
    }

    return filtered;
  }, [females, normalizedPTAs, selectedPTAs]);

  const processedTrendData = useMemo(() => {
    if (!Array.isArray(females) || females.length === 0) return [];
    if (validSelectedPTAs.length === 0) return [];
  ...
  }, [females, groupBy, validSelectedPTAs]);
