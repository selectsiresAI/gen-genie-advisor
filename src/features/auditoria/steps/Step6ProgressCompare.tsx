...
/* ============= Sinônimos simples por segurança ============= */
...
function resolveTraitValue(row: any, canonical: string): number | null {
  // Mapa normalizado da linha
  const nm: Record<string, any> = {};
  for (const k of Object.keys(row)) nm[norm(k)] = row[k];

  const candidates = PTA_SYNONYMS_NORM[canonical] || [norm(canonical)];
  for (const k of candidates) {
    if (k in nm) {
      const v = toNumber(nm[k]);
      if (v != null) return v;
    }
    // tentativas simples com "dollar"
...
export default function Step6ProgressCompare() {
  ...
  const fetchRebanho = useCallback(async () => {
    ...
    // Busca com filtro por farm_id; fallback por id_fazenda; se vazio, nada é mostrado.
    ...
    // Categoria: usa exatamente "Categoria", com fallback para "categoria" (case-insensitive)
    const sample = data[0] || {};
    const categoryKey =
      "Categoria" in sample
        ? "Categoria"
        : Object.keys(sample).find((k) => norm(k) === norm("categoria")) || "Categoria";

    const cats = Array.from(
      new Set(
        data
          .map((r: any) => r?.[categoryKey])
    ...
    // Ajusta grupos padrão se necessário
  ...
  /* =========== 2) Médias por Categoria (rebanho.Categoria) =========== */
  const meansByCategory: MeansByCategory = useMemo(() => {
    if (!rows.length) return {};
    // Detecta chave de categoria novamente para garantir consistência
    const sample = rows[0] || {};
    const categoryKey =
      "Categoria" in sample
        ? "Categoria"
    ...
    for (const r of rows) {
      const cat = (r?.[categoryKey] as string) || "Sem categoria";
      if (!acc[cat]) acc[cat] = {};
      for (const t of ALL_PTA_KEYS) {
        const v = resolveTraitValue(r, t);
        if (v != null) {
          if (!acc[cat][t]) acc[cat][t] = { sum: 0, n: 0 };
          acc[cat][t].sum += v;
          acc[cat][t].n += 1;
        }
      }
    ...
    for (const [cat, map] of Object.entries(acc)) {
      ...
      for (const t of ALL_PTA_KEYS) {
        const b = map[t];
        out[cat][t] = b && b.n > 0 ? b.sum / b.n : null;
      }
    }
    return out;
  }, [rows]);

  /* =========== 3) Monta dados para Tabela e Radar =========== */
  const view = useMemo(() => {
