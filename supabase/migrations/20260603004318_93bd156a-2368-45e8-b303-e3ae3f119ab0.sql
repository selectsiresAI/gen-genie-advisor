...
AS
SELECT
  id, naab_code AS code, name, registration, birth_date,
  sire_naab, mgs_naab, mmgs_naab, company,
  hhp_dollar, tpi, nm_dollar, cm_dollar, fm_dollar, gm_dollar,
  f_sav,
  pta_milk AS ptam, cfp,
  pta_fat AS ptaf, pta_fat_pct AS ptaf_pct,
  pta_protein AS ptap, pta_protein_pct AS ptap_pct,
  pta_pl AS pl, pta_dpr AS dpr, pta_livability AS liv, pta_scs AS scs,
  mast, met, rp, da, ket, mf_num AS mf,
  pta_ptat AS ptat, pta_udc AS udc, pta_flc AS flc,
  pta_sce AS sce, pta_sire_sce AS dce,
  ssb, dsb, h_liv,
  pta_ccr AS ccr,
  pta_hcr AS hcr,
  fi, bwc, sta, str_num AS str, dfm,
...
-- Recriar RPCs (CASCADE acima derruba pois retornam bulls_denorm)
...
BEGIN
  ...
  SELECT b.id INTO matched_id FROM public.bulls b
...
AS $function$
DECLARE
  v_naab text; v_normalized text; v_variants text[]; v_suffixes text[]; v_id uuid;
BEGIN
  FOREACH v_naab IN ARRAY p_naabs LOOP
    v_normalized := public.expand_naab_query(v_naab); v_id := NULL;
...
-- Recalcula placeholders agora que CCR/HCR têm dados reais
-- (a função já lê de pta_ccr/pta_hcr via colunas ccr_num/hcr_num? Não — usa ccr_num/hcr_num).
-- Vamos preencher ccr_num/hcr_num dos placeholders manualmente a partir das médias de pta_ccr/pta_hcr.
UPDATE public.bulls b
SET ccr_num = s.ccr_avg, hcr_num = s.hcr_avg, updated_at = now()
FROM (
  SELECT
    (SELECT AVG(pta_ccr) FROM public.bulls WHERE EXTRACT(YEAR FROM birth_date) = 2020) AS ccr_avg_2020,
    (SELECT AVG(pta_hcr) FROM public.bulls WHERE EXTRACT(YEAR FROM birth_date) = 2020) AS hcr_avg_2020,
    (SELECT AVG(pta_ccr) FROM public.bulls WHERE EXTRACT(YEAR FROM birth_date) = 2017) AS ccr_avg_2017,
    (SELECT AVG(pta_hcr) FROM public.bulls WHERE EXTRACT(YEAR FROM birth_date) = 2017) AS hcr_avg_2017
) avgs
CROSS JOIN LATERAL (
  VALUES
    ('007HO00001'::text, avgs.ccr_avg_2020, avgs.hcr_avg_2020),
    ('007HO00002'::text, avgs.ccr_avg_2017, avgs.hcr_avg_2017)
) AS s(naab, ccr_avg, hcr_avg)
WHERE b.naab_code = s.naab;