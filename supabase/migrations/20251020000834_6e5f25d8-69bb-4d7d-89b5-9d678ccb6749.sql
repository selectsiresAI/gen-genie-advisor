...
WHERE NOT EXISTS (SELECT 1 FROM farms f WHERE f.staging_raw_id = sf.raw_id);
...
WHERE NOT EXISTS (
  SELECT 1 FROM user_farms uf WHERE uf.farm_id = f.id
);
...
SELECT 
  ...
  sf.category, sf.sire_naab, sf.mgs_naab, sf.mmgs_naab,
  sf.beta_casein, sf.kappa_casein, 'Importação Staging',
...
FROM staging_females sf
JOIN farms f ON f.staging_raw_id = sf.farm_id
WHERE sf.imported_at IS NULL;

-- Marcar staging_females
