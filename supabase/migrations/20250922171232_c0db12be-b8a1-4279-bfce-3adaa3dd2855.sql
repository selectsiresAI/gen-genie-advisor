...
-- 4. Recriar farm_dashboard_kpis como view normal
...
LEFT JOIN (
    SELECT 
        ...
        AVG(((ptas ->> 'nm_dollar'))::numeric) AS avg_nm_dollar,
        AVG(((ptas ->> 'tpi'))::numeric) AS avg_tpi,
        AVG(((ptas ->> 'hhp_dollar'))::numeric) AS avg_hhp_dollar,
        MAX(created_at) AS last_female_added
...
LEFT JOIN (
    SELECT 
        ...
        MAX(created_at) AS last_mating_date
    FROM public.matings
    GROUP BY farm_id
...
LEFT JOIN (
    SELECT 
        farm_id,
        SUM(CASE WHEN movement_type = 'entrada' THEN quantity ELSE -quantity END) AS total_semen_doses,
        MAX(movement_date) AS last_movement_date
    FROM public.semen_movements
    GROUP BY farm_id
