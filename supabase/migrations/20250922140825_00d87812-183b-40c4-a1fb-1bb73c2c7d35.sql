-- ==========================================
-- 12. RPCs ESSENCIAIS PARA OPERAÇÕES CRÍTICAS
-- ==========================================
...
BEGIN
    ...
    ELSE
        -- Touro não encontrado, buscar sugestões similares
        SELECT ARRAY_AGG(b.code ORDER BY levenshtein(public.normalize_naab(b.code), normalized_naab) LIMIT 5)
        INTO suggestion_array
        FROM public.bulls b
        WHERE levenshtein(public.normalize_naab(b.code), normalized_naab) <= 3;
        
        IF suggestion_array IS NULL THEN
            suggestion_array := ARRAY[]::TEXT[];
        END IF;
...
BEGIN
    ...
    LOOP
        ...
        ELSE
            -- Buscar sugestões
            SELECT ARRAY_AGG(b.code ORDER BY levenshtein(public.normalize_naab(b.code), normalized) LIMIT 5)
            INTO suggestion_array
            FROM public.bulls b
...
BEGIN
    ...
    ELSE
        -- Buscar sugestões
        SELECT ARRAY_AGG(b.code ORDER BY levenshtein(public.normalize_naab(b.code), normalized_naab) LIMIT 3)
        INTO suggestions
        FROM public.bulls b
        WHERE levenshtein(public.normalize_naab(b.code), normalized_naab) <= 3;
