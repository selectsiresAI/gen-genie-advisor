-- Criar função helper para parse de datas flexível
CREATE OR REPLACE FUNCTION public.parse_flexible_date(date_str TEXT)
RETURNS DATE
LANGUAGE plpgsql
IMMUTABLE
AS $$
...
BEGIN
  ...
  -- Se p1 > 12, é DD/MM/YYYY
  IF p1 > 12 THEN
    RETURN make_date(p3, p2, p1);
  END IF;
  
  -- Se p2 > 12, é MM/DD/YYYY
  IF p2 > 12 THEN
    RETURN make_date(p3, p1, p2);
  END IF;
  
  -- Ambíguo: assumir MM/DD/YYYY (padrão americano)
  RETURN make_date(p3, p1, p2);
  
EXCEPTION WHEN OTHERS THEN
  -- Se falhar, tentar DD/MM/YYYY
  BEGIN
    RETURN make_date(p3, p2, p1);
  EXCEPTION WHEN OTHERS THEN
    RETURN NULL;
...
-- Agora migrar com a função robusta
